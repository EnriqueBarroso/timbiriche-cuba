import { getProductById, getBusinessByOwnerId } from "@/lib/api";
...
export default async function EditPage({ params }: Props) {
  ...
  const [product, currentBusiness] = await Promise.all([
    getProductById(id).catch(() => null),
    getBusinessByOwnerId(user.id),
  ]);

  if (!product) return notFound();
  if (!currentBusiness || currentBusiness.id !== product.businessId) return redirect("/");

  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        {/* 🛡️ LA MAGIA ESTÁ AQUÍ */}
        {product.business?.isRestaurant ? (
          <>
